...
- [](/exchange/antispam-and-antimalware/antispam-and-antimalware) 스팸 방지 및 맬웨어 방지 보호(Exchange 관리 센터 관리 인터페이스에 직접 액세스)**:** 기본 제공 스팸 방지 및 맬웨어 방지 보호(여러 맬웨어 방지 엔진을 사용하여 인바운드, 아웃바운드 및 내부 메시지에서 맬웨어를 검색), 사용자 지정된 스팸 방지 및 맬웨어 방지 정책, 검역 - 관리자 관리 및 검역 - 최종 사용자 자체 관리.
- [**Exchange Online 설정**](/exchange/architecture/client-access/exchange-admin-center)및 관리 **:** Microsoft 365 포털 및 관리 센터 액세스, Exchange 관리 센터 액세스, 원격 Windows PowerShell 액세스, 모바일 장치에 대한 ActiveSync [정책,](/exchange/monitoring/monitoring)사용 현황 보고 .
...
- [**상호운용성,**](/exchange/security-and-compliance/interoperability-connectivity-and-compatiblity)연결성 및 호환성 **:** 비즈니스용 Skype OWA 및 Outlook, SharePoint 상호운용성, EWS 연결 지원(가장에 적용된 EWS 앱 지원), SMTP 릴레이 지원.
...
- [**계획 및**](/exchange/plan-and-deploy/plan-and-deploy)**배포:** [](/exchange/hybrid-deployment/hybrid-deployment) 지원되는 하이브리드 배포(CA 또는 Enterprise SKU로 업그레이드하면 비즈니스 및 F3 계획에 대한 액세스 권한), [IMAP,](/exchange/mailbox-migration/migrating-imap-mailboxes/migrating-imap-mailboxes)전환 및 단계적 마이그레이션이 [지원됩니다.](/exchange/mailbox-migration/what-to-know-about-a-staged-migration)
- [**보고 기능**](/office365/servicedescriptions/exchange-online-service-description/reporting-features-and-troubleshooting-tools)및 문제 해결 **도구:** Microsoft 365 관리 센터 보고서, Excel 통합 문서, 웹 서비스 보고서, 통합 메시징 보고서(E3/E5 및 계획 2만 해당) 메시지 추적 및 감사 [보고서는](/exchange/exchange-auditing-reports-exchange-2013-help) EAC(Exchange 관리 인터페이스)에 직접 액세스하여 액세스할 수 있습니다.
- [**받는**](/exchange/recipients-in-exchange-online/recipients-in-exchange-online)**사람:** 용량 알림, 클러터, 메일 설명, 받은 편지함 규칙, 오프라인 주소 기록, 주소 기록 정책, 메일 그룹, 외부 연락처(전역), 유니버설 연락처 카드, 소셜 네트워크와의 연락처 연결, 리소스 사서함, 회의실 관리, 부재 중 회신, 일정 공유(키오스크 일정은 OWA를 통해서만 액세스하거나 공유할 수 있습니다).
...
||[Azure Information Protection<sup>6을</sup>사용한 IRM,](/exchange/policy-and-compliance/in-place-archiving/in-place-archiving)Office 365 메시지 암호화<sup>7</sup>및 고객 키<sup>8</sup>|아니요|아니요|예|아니요|아니요|아니요|
||In-Place, 소송 자료 보호 및 데이터 손실 방지|아니요|아니요|예|아니요|예<sup>5</sup>|예|
||보관된 데이터 암호화(BitLocker), MRM(메시징 레코드 관리), S/MIME, 저널링, In-Place eDiscovery<sup>9,</sup>전송 규칙<sup>10,</sup>Windows Server AD RMS를 사용하는 IRM<sup></sup>|예|예|예|예|예|예|
...
||연결된 계정<sup>12,</sup>주소 목록 및 전체 주소 목록<sup>13</sup>|예|예|예|예|예|예|
||비활성 사서함|아니요|아니요|예|아니요|예<sup>5</sup>|아니요|
||Microsoft 365 그룹|예|예|예|예|예<sup>14</sup>|아니요|
...
<sup>12</sup> 연결된 계정은 POP & IMAP 계정에 대해 지원되지만 Outlook.com(Hotmail)에는 사용하지 않도록 설정됩니다. <br/>
<sup>13</sup> Exchange Online PowerShell에서 cmdlet을 사용하여 기본 주소 목록 및 기본 GAL(전체 주소 목록)을 사용자 지정할 수 없습니다. <br/>
<sup>14 Microsoft 365</sup> 기능이 있는 그룹을 사용할 수 있습니다. <br/>
...
- **Exchange Server:** Exchange Server 대한 자세한 내용은 Exchange Server 기능 가용성 [표를 참조하세요.](/Exchange/new-features/feature-availability)
...
- Exchange 관리 센터의 새로운 **기능:** Exchange 관리 센터의 새로운 기능에 대한 자세한 내용은 Exchange 관리 센터의 새로운 [기능을 참조하세요.](/exchange/whats-new)
- Exchange Online 시스템 요구 **사항:** 시스템 요구 사항, 비즈니스, 교육 및 정부 조직에서 사용할 수 있는 월별 구독 기반 서비스는 Microsoft 365 및 Office [리소스를 참조하세요.](https://products.office.com/office-system-requirements/#Office365forBEG)
- Storage 대한 Exchange Online 및 받는 사람 제한: Exchange Online 구독 계획에서 사용할 수 있는 저장소 및 받는 사람 제한에 대한 자세한 내용은 Exchange Online [제한을 참조하세요.](/office365/servicedescriptions/exchange-online-service-description/exchange-online-limits)
