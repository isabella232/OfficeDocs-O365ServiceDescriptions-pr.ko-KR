...
관리 역할 그룹은 관리 역할을 관리자 또는 전문가 사용자 그룹에 연결합니다. 관리자는 광범위한 조직 Exchange Online 받는 사람 구성을 관리합니다. 전문가 사용자는 규정 준수와 같은 Exchange Online 기능을 관리하거나 지원 센터 구성원과 같은 관리 기능이 제한될 수 있지만 광범위한 관리 권한을 부여하지는 않습니다. 역할 그룹은 일반적으로 관리자와 전문가 사용자가 조직 및 받는 사람의 구성을 관리할 수 있도록 하는 관리 관리 역할을 연결합니다. 예를 들어 관리자가 받는 사람을 관리할 수 있는지 또는 사서함 검색 기능을 사용할 수 있는지 여부는 역할 그룹을 사용하여 제어합니다. 
