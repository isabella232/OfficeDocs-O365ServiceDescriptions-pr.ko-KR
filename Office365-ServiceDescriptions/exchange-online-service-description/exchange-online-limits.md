...
> [!NOTE]
> <sup>1</sup> Azure Active Directory DirSync를 사용하는 경우 온-프레미스 Active Directory Azure Active Directory 동기화할 수 있는 최대 메일 그룹 멤버 수는 15,000개입니다. [Azure AD 커넥트](/azure/active-directory/hybrid/how-to-connect-sync-endpoint-api-v2) 사용하는 경우 해당 수는 250,000이지만 메일 그룹 구성원의 지원되는 제한은 여전히 100,000입니다. <br/> 
...
| 그룹 사서함 | 50GB | 50GB | 50GB | 50 GB | 50GB | 50GB | 50GB |
...
- **첨부 파일 제한**: 전자 메일 메시지에 허용되는 최대 첨부 파일 수입니다. 모든 파일 첨부 파일의 총 크기가 메시지 크기 제한을 위반하지 않더라도 메시지에 허용되는 첨부 파일 수에는 제한이 있습니다. 이 제한은 여러 부분으로 구성된 메시지 제한을 통해 제어됩니다.
...
- **조정을 기다리는 메시지의 만료**: 기본적으로 조정을 기다리는 메시지는 2일 후에 만료되며 관리자는 이 간격을 연장할 수 없습니다. 그러나 만료된 중재 메시지의 처리가 7일마다 실행됩니다. 이는 2~9일 사이에 언제든지 중재된 메시지가 만료될 수 있음을 의미합니다.
...
- **수신 제한:** 이 제한은 *모든 원본에서* 시간당 메시지 수를 계산합니다. 여기에는 내부 발신자가 보내는 메시지, 인터넷에서 발송되는 메시지, 온-프레미스 서버에서 발송되는 메시지가 모두 포함됩니다. 사서함의 수신 한도를 초과하면 인터넷 또는 온-프레미스 보낸 사람이 사서함으로 보낸 메시지는 사서함이 최대 배달 임계값을 초과했음을 나타내는 배달 못 함 보고서(NDR 또는 반송 메시지라고도 함)에서 보낸 사람에게 반환됩니다. 내부 보낸 사람의 메시지는 제한에 대해 계산되지만 수신 한도를 초과하면 차단되지 않습니다. 한 시간 후에 이 제한이 새로 고쳐지고 사서함은 메시지를 받을 수 있게 됩니다.
- **발신자-수신자 쌍 제한:** 이 제한은 *단일 발신자* 가 보내는 시간당 메시지 수에 적용됩니다. 이것은 단일 발신자 메일 폭풍으로부터 보호하기 위해 전체 수신 제한의 비율로 설정됩니다.

| 기능 | Microsoft 365 Business Basic | Microsoft 365 Business Standard | Microsoft 365 Business Premium | Office 365 Enterprise E1 | Office 365 Enterprise E3 | Office 365 Enterprise E5 | Office 365 Enterprise F3 |
|---------|------------------------------|----------------------------------------|-----------------------------------------|--------------------------|--------------------------|--------------------------|--------------------------|
| 메시지 수신됨 | 시간당 3600개 메시지 | 시간당 3600개 메시지 | 시간당 3600개 메시지 | 시간당 3600개 메시지 | 시간당 3600개 메시지 | 시간당 3600개 메시지 | 하루에 3600개 메시지 |
| 단일 발신자로부터 받은 메시지 | 수신된 메시지의 33% | 수신된 메시지의 33% | 수신된 메시지의 33% | 수신된 메시지의 33% | 수신된 메시지의 33% | 수신된 메시지의 33% | 수신된 메시지의 33% |
...
> <sup>3</sup> 이 수는 문자 수에 따라 약간씩 달라질 수 있으므로 전자 메일 주소 수는 길이에 따라 달라집니다. 일반적으로 개체와 해당 생성 특성이 더 늘어날 가능성을 고려해 ProxyAddresses SMTP 주소 제한은 300개라고 가정하는 게 안전합니다.</br>
> <sup>4</sup> 아웃바운드 메시지 볼륨이 메시지 속도 제한을 초과하는 경우 모든 초과 메시지 제출이 제한되고, 계속해서 다음 시간(분)으로 전달됩니다. 일반적으로 이 작업은 보낸 사람 계정을 차단하지는 않지만 Exchange Online은 대량 메일 발송 시나리오를 수용하는 경우 적합하지 않습니다. 이 사용 사례의 경우 [여기](/exchange/mail-flow-best-practices/how-to-set-up-a-multifunction-device-or-application-to-send-email-using-microsoft-365-or-office-365)에 있는 옵션 2와 3을 대신 사용하는 것이 좋습니다.
