...
Single Sign-On을 사용하는 조직의 경우 도메인의 모든 사용자가 동일한 ID 시스템을 사용해야 합니다. 클라우드 ID 또는 페더레이션 ID 중 하나의 ID 시스템을 사용해야 합니다. 예를 들어 온-프레미스 시스템에 액세스할 필요가 없어 클라우드 ID만 필요한 사용자로 이루어진 그룹과 Office 365 및 온-프레미스 시스템을 모두 사용하는 사용자로 이루어진 그룹이 있습니다. 이 경우 contractors.contoso.com 및 staff.contoso.com과 같은 도메인 2개를 Office 365에 추가하고 이 중 하나에 SSO를 설정할 수 있습니다. 전체 도메인은 클라우드 ID에서 페더레이션 ID로, 또는 페더레이션 ID에서 클라우드 ID로 변환할 수 있습니다.
...
파트너는 고객을 대신하여 계정을 관리할 수 있는 권한을 받을 수 있습니다. 고객에게는 파트너 사용을 위한 사용자 계정이 필요하지 않으며, 관리 권한의 위임을 허용할 때 Office 365 라이선스를 사용하지 않습니다. 파트너는 조직 내 사용자에게 전체 또는 제한된 액세스를 할당할 수 있습니다. 제한된 액세스에는 암호 다시 설정, 서비스 요청 관리 및 서비스 상태 모니터링을 수행할 수 있는 권한이 포함됩니다. 자세한 내용은 [위임된 관리 추가 또는 삭제](https://go.microsoft.com/fwlink/p/?LinkID=270055)를 참조하세요.
